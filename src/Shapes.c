...
Circle_h_t Circle__init(coord_t center, PIXELS radius, unsigned int color)
{
	...
	this->_positionProperties =
		PositionProperties__init(center.x, center.y, color);
	this->_radius = radius;
...
{
	PositionProperties__destruct(
		&((struct Circle *)*handle)->_positionProperties);
			      ...
			      unsigned int color)
{
	...
	this->_positionProperties =
		PositionProperties__init(center.x, center.y, color);
	this->_topLeftCorner =
		(coord_t){ center.x - width / 2, center.y - height / 2 };
	this->_width = width;
	this->_height = height;
...
{
	PositionProperties__destruct(
		&((struct Rectangle *)*handle)->_positionProperties);
...
int Rectangle__updateCorner(Rectangle_h_t handle)
{
	...
	this->_topLeftCorner =
		(coord_t){ PositionProperties__getX(this->_positionProperties) -
				   this->_width / 2,
			   PositionProperties__getY(this->_positionProperties) -
				   this->_height / 2 };
...
Triangle_h_t Triangle__init(coord_t center, PIXELS height, unsigned int color)
{
	...
	this->_corners[1] =
		(coord_t){ center.x - height / 2, center.y + height / 2 };
	this->_corners[2] =
		(coord_t){ center.x + height / 2, center.y + height / 2 };
...
{
	PositionProperties__destruct(
		&((struct Triangle *)*handle)->_positionProperties);
...
Message_h_t Message__init(coord_t center, char *text, unsigned int color)
{
	...
	this->_topLeftCorner = (coord_t){ center.x - this->_textWidth / 2,
					  center.y - this->_textHeight / 2 };
...
{
	PositionProperties__destruct(
		&((struct Message *)*handle)->_positionProperties);
				       ...
				       unsigned int color)
{
	...
	this->_positionProperties = PositionProperties__init(
		topLeftCorner.x + this->_textWidth / 2,
		topLeftCorner.y + this->_textHeight / 2, color);

...
PositionProperties_h_t PositionProperties__init(PIXELS x, PIXELS y,
						unsigned int color)
{
	struct PositionProperties *this =
		malloc(sizeof(struct PositionProperties));
				       ...
				       unsigned int timePassed_ms)
{
	...
	PositionProperties__setPosition(handle,
					this->_f_x + this->_dx * timePassed_s,
					     ...
					     unsigned int timePassedInTotal_ms)
{
	...
	this->_dx =
		(-1) *
		sin((2 * M_PI * timePassedInTotal_ms) / timePeriod_ms + phase) *
		(radius * 2 * M_PI) / timePeriod_s;
	this->_dy =
		(-1) *
		cos((2 * M_PI * timePassedInTotal_ms) / timePeriod_ms + phase) *
		(radius * 2 * M_PI) / timePeriod_s;
...
void PositionProperties__printPositionAndSpeed(PositionProperties_h_t handle)
{
	...
	printf("Coordinates rounded: %d %d,  exact: %f %f,  Speed: %f %f\n",
	       this->_x, this->_y, this->_f_x, this->_f_y, this->_dx,
	       this->_dy);
}
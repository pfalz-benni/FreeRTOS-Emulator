...
void vDrawFPS(void)
{
	...
	if (!tumGetTextSize((char *)str, &text_width, NULL)) {
		tumDrawText(str, SCREEN_WIDTH - text_width - 10,
			    SCREEN_HEIGHT - DEFAULT_FONT_SIZE * 1.5, Skyblue);
	}
			       ...
			       buttonPresses_t buttonPressCountNS)
{
	...
	if (xSemaphoreTake(buttonPressCountNS.lock, portMAX_DELAY) == pdTRUE) {
		sprintf(formatedText,
			"N: %u |  S: %u           Seconds passed: %d",
	...
	tumDrawText(Message__getText(buttonsNSPressMessage),
		    Message__getTopLeftCorner(buttonsNSPressMessage).x,
		    Message__getTopLeftCorner(buttonsNSPressMessage).y,
		    PositionProperties__getColor(Message__getPositionProperties(
			    buttonsNSPressMessage)));
}

void vBlinkingButtonsDrawTask(void *pvParameters)
{
	Circle_h_t circleDynamic = Circle__init(
		(coord_t){ SCREEN_CENTER.x - DISTANCE_CIRCLE_CENTER,
			   SCREEN_CENTER.y },
		RADIUS_CIRCLES, Red);

	Circle_h_t circleStatic = Circle__init(
		(coord_t){ SCREEN_CENTER.x + DISTANCE_CIRCLE_CENTER,
			   SCREEN_CENTER.y },
		RADIUS_CIRCLES, Blue);

	Message_h_t buttonsNSPressMessage = Message__init(
		(coord_t){ SCREEN_CENTER.x,
	...
	while (1) {
		...
		if (notification & 0x01) {
			tumDrawCircle(PositionProperties__getX(
					      Circle__getPositionProperties(
						      circleDynamic)),
				      PositionProperties__getY(
					      Circle__getPositionProperties(
						      circleDynamic)),
		...
		drawButtonsNSPressMessage(buttonsNSPressMessage,
					  buttonPressCountNS);

		xSemaphoreGive(ScreenLock);

		vTaskDelay(portTICK_PERIOD_MS *
			   TIME_BLINKINGBUTTONSDRAWTASK_DELAY_MS);
	}
}
...
void vButtonPressSemaphoreTask(void *pvParameters)
{
	while (1) {
		if (xSemaphoreTake(ButtonSPressed, portMAX_DELAY) == pdTRUE) {
			if (xSemaphoreTake(buttonPressCountNS.lock,
...
{
	while (1) {
		if (xSemaphoreTake(secondsPassedTotal.lock,
				   portMAX_DELAY == pdTRUE)) {

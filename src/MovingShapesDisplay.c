...
//As a result the adresses pointed to have to be edited, hence be passed by reference
void resetMovingObjectsAfterResumption(Circle_h_t *circle,
				       Rectangle_h_t *rectangle,
				       Message_h_t *topMessage,
				       coord_t *mobileScreenCenter,
...
void updateShapeAndMessagePositions(Circle_h_t circle, Rectangle_h_t rectangle,
				    ...
				    TickType_t initialWakeTime)
{
	PositionProperties__setSpeedMoveOnCircle(
		Circle__getPositionProperties(circle), DISTANCE_TO_TRIANGLE,
		M_PI, TIME_ORBIT_PERIOD, xLastWakeTime - initialWakeTime);
	PositionProperties__updatePosition(
		Circle__getPositionProperties(circle),
		xLastWakeTime - prevWakeTime);

	PositionProperties__setSpeedMoveOnCircle(
		Rectangle__getPositionProperties(rectangle),
		DISTANCE_TO_TRIANGLE, 0, TIME_ORBIT_PERIOD,
	...
	PositionProperties__moveVetically(
		Message__getPositionProperties(topMessage),
		SPEED_MOVING_MESSAGE, DISTANCE_HORIZONTAL_TO_BORDER);
	PositionProperties__updatePosition(
		Message__getPositionProperties(topMessage),
		xLastWakeTime - prevWakeTime);
...
{
	tumDrawCircle(
		PositionProperties__getX(Circle__getPositionProperties(circle)),
		PositionProperties__getY(Circle__getPositionProperties(circle)),
		Circle__getRadius(circle),
		PositionProperties__getColor(
			Circle__getPositionProperties(circle)));
	...
	tumDrawTriangle(Triangle__getCorners(triangle),
			PositionProperties__getColor(
				Triangle__getPositionProperties(triangle)));
}

void drawSimpleTextMessages(Message_h_t topMessage, Message_h_t bottomMessage)
{
	tumDrawText(Message__getText(bottomMessage),
		    Message__getTopLeftCorner(bottomMessage).x,
		    Message__getTopLeftCorner(bottomMessage).y,
		    PositionProperties__getColor(
			    Message__getPositionProperties(bottomMessage)));

	tumDrawText(Message__getText(topMessage),
		    Message__getTopLeftCorner(topMessage).x,
			    ...
			    buttonPresses_t buttonPressCountABCD)
{
	...
	if (xSemaphoreTake(buttonPressCountABCD.lock, portMAX_DELAY) ==
	    pdTRUE) {
		sprintf(formatedText, "A: %u |  B: %u |  C: %u |  D: %u",
			buttonPressCountABCD.values[0],
			buttonPressCountABCD.values[1],
			buttonPressCountABCD.values[2],
			buttonPressCountABCD.values[3]);
		xSemaphoreGive(buttonPressCountABCD.lock);
	}

	Message__setText(buttonPressMessage, formatedText);

	tumDrawText(Message__getText(buttonPressMessage),
		    Message__getTopLeftCorner(buttonPressMessage).x,
		    Message__getTopLeftCorner(buttonPressMessage).y,
		    PositionProperties__getColor(Message__getPositionProperties(
			    buttonPressMessage)));
}
...
void drawMouseCoordMessage(Message_h_t mouseCoordMessage)
{
	...
	sprintf(formatedText, "X-axis: %d |  Y-axis: %d", tumEventGetMouseX(),
		tumEventGetMouseY());
	Message__setText(mouseCoordMessage, formatedText);

	tumDrawText(Message__getText(mouseCoordMessage),
		    Message__getTopLeftCorner(mouseCoordMessage).x,
		    Message__getTopLeftCorner(mouseCoordMessage).y,
		    PositionProperties__getColor(
			    Message__getPositionProperties(mouseCoordMessage)));
}
				 ...
				 Message_h_t mouseCoordMessage)
{
	...
	mobileScreenCenter->x =
		SCREEN_CENTER.x - (SCREEN_CENTER.x - tumEventGetMouseX()) /
					  (double)RATION_CURSER_CENTER;
	mobileScreenCenter->y =
		SCREEN_CENTER.y - (SCREEN_CENTER.y - tumEventGetMouseY()) /
					  (double)RATION_CURSER_CENTER;

	// update all objects' coordinates according to new screen center
	PositionProperties__adjustPositionToNewScreenCenter(
		Circle__getPositionProperties(circle), oldScreenCenter,
		*mobileScreenCenter);

	PositionProperties__adjustPositionToNewScreenCenter(
		Rectangle__getPositionProperties(rectangle), oldScreenCenter,
		*mobileScreenCenter);
	Rectangle__updateCorner(rectangle);

	PositionProperties__adjustPositionToNewScreenCenter(
		Triangle__getPositionProperties(triangle), oldScreenCenter,
		*mobileScreenCenter);
	Triangle__updateCorners(triangle);

	PositionProperties__adjustPositionToNewScreenCenter(
		Message__getPositionProperties(bottomMessage), oldScreenCenter,
		*mobileScreenCenter);
	Message__updateCorner(bottomMessage);

	PositionProperties__adjustPositionToNewScreenCenter(
		Message__getPositionProperties(topMessage), oldScreenCenter,
		*mobileScreenCenter);
	Message__updateCorner(topMessage);

	PositionProperties__adjustPositionToNewScreenCenter(
		Message__getPositionProperties(buttonPressMessage),
		oldScreenCenter, *mobileScreenCenter);
...
void vMovingShapesDisplayTask(void *pvParameters)
{
	...
	// Create three shapes
	Circle_h_t circle = Circle__init(
		(coord_t){ mobileScreenCenter.x - DISTANCE_TO_TRIANGLE,
			   mobileScreenCenter.y },
		RADIUS_CIRCLE, TUMBlue);

	Rectangle_h_t rectangle = Rectangle__init(
		(coord_t){ mobileScreenCenter.x + DISTANCE_TO_TRIANGLE,
			   mobileScreenCenter.y },
		HEIGHT_SQUARE, HEIGHT_SQUARE, Olive);

	Triangle_h_t triangle =
		Triangle__init(mobileScreenCenter, HEIGHT_TRIANGLE, Magenta);

	//Create messages
	Message_h_t bottomMessage = Message__init(
		(coord_t){ mobileScreenCenter.x,
			   SCREEN_HEIGHT - DISTANCE_VERTICAL_TO_BORDER },
		"Hang loose or press [Q] to quit!", Black);

	Message_h_t topMessage = Message__init(
		(coord_t){ mobileScreenCenter.x,
			   mobileScreenCenter.y - SCREEN_CENTER.y +
				   DISTANCE_VERTICAL_TO_BORDER },
		"Hello, I can move!", Black);
	...
	Message_h_t buttonPressMessage =
		Message__initTopLeftCorner(COORD_BUTTON_PRESS_MESSAGE,
					   "A: 0 |  B: 0 |  C: 0 |  D: 0",
					   Black);

	Message_h_t mouseCoordMessage = Message__initTopLeftCorner(
		(coord_t){ COORD_BUTTON_PRESS_MESSAGE.x,
			   COORD_BUTTON_PRESS_MESSAGE.y +
				   Message__getTextHeight(buttonPressMessage) },
	...
	while (1) {
		...
		moveScreenInCursorDirection(&mobileScreenCenter, circle,
					    rectangle, triangle, bottomMessage,
					    topMessage, buttonPressMessage,
					    mouseCoordMessage);

		//update positions before drawing objcts
		updateShapeAndMessagePositions(circle, rectangle, topMessage,
					       xLastWakeTime, prevWakeTime,
					       initialWakeTime);

		//reset moving objects if task has been resumed after
		//it has been suspended
		resetMovingObjectsAfterResumption(
			&circle, &rectangle, &topMessage, &mobileScreenCenter,
			&xLastWakeTime, &initialWakeTime);
		...
		drawButtonPressMessage(buttonPressMessage,
		...
		vTaskDelay(portTICK_PERIOD_MS *
			   TIME_MOVINGSHAPESDISPLAYTASK_DELAY_MS);

		prevWakeTime =
			xLastWakeTime; // to keep track of time intervalls
	}
}